...
def run_skf_conv1(inputs):
    ...
    write_dir = create_results_directory('./results/skf',
                                         ...
                                         excels=['skf_results'])
    fl = load_data_to_fl('./excel/Data_loader_spline_full_onehot_R4_arcsinh.xlsx', normalise_labels=False,
                         norm_mask=[0, 0, 0, 1, 1, 1])
    fl_store = fl.create_kf(k_folds=10, shuffle=True)

    run_skf(model_mode='conv1', loss_mode='ann', fl=fl, fl_store=fl_store, hparams=hparams,
            norm_mask=[0,0,0,1,1,1], normalise_labels=True, labels_norm=False,
